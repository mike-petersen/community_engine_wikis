class WikisController < BaseController
	...
	before_filter :wiki_membership_required, :only => [:show]
	...
	show.before do
		@comments = @wiki.wiki_pages.find(:all, :page => {:size => 10, :current => params[:page]})
	end
	...
	def build_object
      @object ||= current_user.wikis.build object_params
